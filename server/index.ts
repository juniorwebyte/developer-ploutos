...
app.post('/api/public/reset-password', async (req, res) => {
  try {
    ...
    let resets: any[] = [];
    try {
      const content = fs.readFileSync('reset_requests.json', 'utf-8');
...
app.post('/api/public/reset-username', async (req, res) => {
  try {
    ...
    let resets: any[] = [];
    try {
      const content = fs.readFileSync('username_reset_requests.json', 'utf-8');
      resets = JSON.parse(content);
    } catch (err) {
      resets = [];
    }
...
app.get('/api/cms/config', authMiddleware(), requireRole('admin','superadmin'), async (req, res) => {
  try {
    let config: any = {};
    try {
      const content = fs.readFileSync(CMS_CONFIG_FILE, 'utf-8');
