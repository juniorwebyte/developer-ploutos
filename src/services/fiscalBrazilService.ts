...
class FiscalBrazilService {
  ...
  /**
   * Mapeia forma de pagamento do PDV/Movimento de Caixa para código da SEFAZ (NFC-e/NFe).
   * 01=Dinheiro, 02=Cheque, 03=Cartão Crédito, 04=Cartão Débito, 15=Boleto, 17=PIX, etc.
   */
  private paymentMethodToTipoFiscal(paymentMethod: string): string {
    const m = (paymentMethod || '').toString().toLowerCase();
    const map: Record<string, string> = {
      'dinheiro': '01', 'cash': '01', 'money': '01',
      'cheque': '02', 'check': '02',
      'cartao': '03', 'credit_card': '03', 'debit_card': '04', 'cartao link': '03', 'cartaolink': '03',
      'pixmaquininha': '17', 'pixconta': '17', 'pix': '17', 'pix maquininha': '17', 'pix conta': '17',
      'boleto': '15', 'boletos': '15', 'bank_slip': '15'
    };
    return map[m] || (/\d{2}/.test(m) ? m : '01');
  }
  ...
  generateNFCe(saleData: any, emitenteData: any): NFCeData {
    ...
    const nfce: NFCeData = {
      ...
      formaPagamento: [{
        tipo: this.paymentMethodToTipoFiscal(saleData.paymentMethod),
