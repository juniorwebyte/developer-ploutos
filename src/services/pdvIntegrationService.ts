...
class PDVIntegrationService {
  ...
  /**
   * Normaliza o método de pagamento para o padrão do Movimento de Caixa.
   * Garante compatibilidade entre PDV (cash, credit_card, etc.) e caixa (dinheiro, cartao, pixMaquininha, etc.)
   */
  private normalizePaymentMethod(method: string): PDVSale['paymentMethod'] {
    const m = (method || '').toString().toLowerCase().replace(/\s+/g, '');
    const map: Record<string, PDVSale['paymentMethod']> = {
      'cash': 'dinheiro', 'dinheiro': 'dinheiro', 'money': 'dinheiro',
      'credit_card': 'cartao', 'debit_card': 'cartao', 'cartao': 'cartao', 'cartão': 'cartao', 'card': 'cartao', 'credito': 'cartao', 'debito': 'cartao',
      'cartaolink': 'cartaoLink', 'cartao_link': 'cartaoLink', 'cartao link': 'cartaoLink',
      'pix': 'pixMaquininha', 'pixmaquininha': 'pixMaquininha', 'pix_maquininha': 'pixMaquininha',
      'pixconta': 'pixConta', 'pix_conta': 'pixConta', 'pix conta': 'pixConta',
      'boleto': 'boleto', 'boletos': 'boleto', 'bank_slip': 'boleto',
      'cheque': 'cheque', 'check': 'cheque'
    };
    return map[m] || 'dinheiro';
  }

  /**
   * Lê vendas salvas pelo PDV interno (localStorage 'pdv_sales') e converte para PDVSale[].
   */
  private readSalesFromPdvStorage(startDate?: Date, endDate?: Date): PDVSale[] {
    try {
      const raw = typeof localStorage !== 'undefined' ? localStorage.getItem('pdv_sales') : null;
      if (!raw) return [];
      ...
      const parseDate = (d: string): string => {
        ...
        const parts = (d || '').split(/[/-]/);
        if (parts.length >= 3) {
          const day = parts[0].padStart(2, '0'), month = parts[1].padStart(2, '0'), year = parts[2].length === 4 ? parts[2] : '20' + parts[2];
          return `${year}-${month}-${day}`;
        }
        return today;
      };
      const sales: PDVSale[] = arr.map((s: any) => ({
        id: s.id || `pdv_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`,
        date: parseDate(s.date),
        time: (s.time || '00:00:00').toString().slice(0, 8),
        total: Number(s.total) || 0,
        paymentMethod: this.normalizePaymentMethod(s.paymentMethod),
        items: Array.isArray(s.items) ? s.items.map((i: any) => ({
          ...
          total: Number(i.total_price ?? i.total ?? i.totalPrice) || 0
        })) : []
      })).filter(s => s.total > 0);
      ...
      return todaySales.length > 0 ? todaySales : sales;
    } catch {
      return [];
    }
  }

  // Importar vendas do PDV (PDV interno em pdv_sales, API quando configurado, ou cache)
    ...
    try {
      ...
      if (config.pdvEndpoint && config.pdvEndpoint.trim() !== '' && config.enabled) {
        try {
          ...
          // if (response.ok) { const apiSales = await response.json(); this.addMockSales(apiSales); sales = apiSales; }
        } catch (apiError) {
          // Se falhar, usar fontes locais
        }
      }
      
      // Prioridade 1: vendas do PDV interno (pdv_sales) — sincronização com Movimento de Caixa
      const fromPdv = this.readSalesFromPdvStorage(startDate, endDate);
      if (fromPdv.length > 0) {
        sales = fromPdv;
      } else {
        // Prioridade 2: cache de vendas (pdv_sales_cache)
        const cached = localStorage.getItem(this.SALES_KEY);
        if (cached) {
          const cachedSales = JSON.parse(cached) as PDVSale[];
          for (const s of cachedSales) {
            s.paymentMethod = this.normalizePaymentMethod(s.paymentMethod as string);
          }
          if (startDate && endDate) {
