...
import { CashFlowData, Sangria } from '../types';
...
export function printCashFlow(data: CashFlowData, reduced: boolean = false, incluirObservacoes: boolean = false) {
  ...
  const totalSangrias = Array.isArray(data.exits.sangrias)
    ? data.exits.sangrias.reduce((sum, s) => sum + (Number(s.valor) || 0), 0)
    : 0;
  ...
  const htmlContent = reduced
    ? generateReducedHTML(data, totalEntradas, totalSaidas, totalSangrias, incluirObservacoes)
    : generateFullHTML(data, totalEntradas, totalSaidas, totalDevolucoes, valesImpactoEntrada, totalCheques, totalEnviosCorreiosEntrada, totalTaxas, totalVRLancamentos, totalVALancamentos, totalSangrias, incluirObservacoes);
...
function generateFullHTML(data: CashFlowData, totalEntradas: number, totalSaidas: number, totalDevolucoes: number, valesImpactoEntrada: number, totalCheques: number, totalEnviosCorreiosEntrada: number, totalTaxas: number, totalVRLancamentos: number, totalVALancamentos: number, totalSangrias: number, incluirObservacoes: boolean = false): string {
  return `
<!DOCTYPE html>
<html>
<head>
...
<body>
  ...
  <div class="section">
    ...
    ${Array.isArray(data.exits.puxadores) && data.exits.puxadores.length > 0
      ...
      : ''}
    ${totalSangrias > 0 ? `<div class="row"><span>Sangria de Caixa:</span><span>${formatCurrency(totalSangrias)}</span></div>` : ''}
    ${Array.isArray(data.exits.sangrias) && data.exits.sangrias.length > 0
      ? `<div>
          ${data.exits.sangrias.map(s => `<div class="row-desc"><span>${new Date(s.dataHora).toLocaleString('pt-BR')}${s.observacao ? ' - ' + s.observacao : ''}</span><span>${formatCurrency(s.valor)}</span></div>`).join('')}
        </div>`
      : ''}
    <div class="total">
...
function generateReducedHTML(data: CashFlowData, totalEntradas: number, totalSaidas: number, _totalSangrias?: number, incluirObservacoes: boolean = false): string {
  return `
<!DOCTYPE html>
<html>
<head>
...
/** Imprime comprovante fiscal individual de uma Sangria de Caixa */
export function printSangriaComprovante(sangria: Sangria, dataDate?: string) {
  const printWindow = window.open('', '_blank', 'width=800,height=600');
  if (!printWindow) {
    alert('Por favor, permita pop-ups para imprimir o comprovante.');
    return;
  }
  const dataHora = new Date(sangria.dataHora);
  const dataStr = dataDate || dataHora.toLocaleDateString('pt-BR');
  const horaStr = dataHora.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
  const html = `
<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
  <title>Cupom Fiscal - Sangria de Caixa</title>
  <style>
    @page { size: 80mm auto; margin: 0; }
    * { margin: 0; padding: 0; box-sizing: border-box; font-weight: bold; }
    body, .container, .header, .section, .section-title, .row, .total, .footer, h1, div, span { font-weight: bold; }
    body { font-family: 'Courier New', monospace; font-size: 12px; padding: 5mm; background: white; }
    ...
    .logo { max-width: 60mm; height: auto; margin: 0 auto 5px; display: block; }
    h1 { font-size: 14px; font-weight: bold; margin: 5px 0; }
    .section { margin: 10px 0; }
    .section-title { background: #f0f0f0; font-weight: bold; padding: 5px; text-align: center; border: 1px solid #333; margin-bottom: 8px; }
    .row { display: flex; justify-content: space-between; margin: 5px 0; font-size: 11px; font-weight: bold; }
    .total { border-top: 2px solid black; border-bottom: 2px solid black; padding: 10px 0; margin: 15px 0; text-align: center; background: #f0f0f0; font-weight: bold; font-size: 14px; }
    .footer { text-align: center; border-top: 2px solid black; padding-top: 8px; margin-top: 15px; font-size: 10px; font-weight: bold; }
  </style>
</head>
<body>
  <div class="container">
    <div class="header">
      <img src="/logo_header.png" alt="Logo" class="logo" onerror="this.style.display='none'">
      <h1>CUPOM FISCAL - SANGRIA DE CAIXA</h1>
      <div>Documento auxiliar</div>
    </div>
    <div class="section">
      <div class="section-title">IDENTIFICAÇÃO</div>
      <div class="row"><span>Data:</span><span>${dataStr}</span></div>
      <div class="row"><span>Hora:</span><span>${horaStr}</span></div>
      <div class="row"><span>ID:</span><span>${sangria.id}</span></div>
      ${sangria.responsavel ? `<div class="row"><span>Responsável:</span><span>${sangria.responsavel}</span></div>` : ''}
    </div>
    <div class="section">
      <div class="section-title">VALOR DA SANGRIA</div>
      <div class="total">${formatCurrency(sangria.valor)}</div>
    </div>
    ${sangria.observacao ? `
    <div class="section">
      <div class="section-title">OBSERVAÇÃO</div>
      <div style="padding: 5px; font-size: 11px; word-wrap: break-word; font-weight: bold;">${sangria.observacao}</div>
    </div>
    ` : ''}
    <div class="footer">
      <div>================================</div>
