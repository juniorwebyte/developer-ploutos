...
function PDVSystemNew({ onClose }: PDVSystemNewProps) {
  ...
  const [customerFormCep, setCustomerFormCep] = useState('');
  const [loadingCustomerCnpj, setLoadingCustomerCnpj] = useState(false);
  const [loadingCustomerCep, setLoadingCustomerCep] = useState(false);
  useEffect(() => {
    if (!showCustomerModalForm) return;
    const clean = (customerFormData.cpf || '').replace(/\D/g, '');
    if (clean.length !== 14) return;
    const t = setTimeout(() => {
      setLoadingCustomerCnpj(true);
      cnpjService.consultarCNPJ(clean).then((data) => {
        if (data) {
          const endereco = [data.logradouro, data.numero, data.bairro, data.municipio, data.uf].filter(Boolean).join(', ');
          setCustomerFormData(prev => ({
            ...
            address: prev.address || endereco || '',
          }));
        }
        setLoadingCustomerCnpj(false);
      }).catch(() => setLoadingCustomerCnpj(false));
    }, 500);
    return () => clearTimeout(t);
  }, [showCustomerModalForm, customerFormData.cpf]);
  useEffect(() => {
    if (!showCustomerModalForm) return;
    const clean = customerFormCep.replace(/\D/g, '');
    if (clean.length !== 8) return;
    const t = setTimeout(() => {
      setLoadingCustomerCep(true);
      cepService.buscarCEP(clean).then((data) => {
        if (data) {
          const endereco = [data.logradouro, data.bairro, data.localidade, data.uf].filter(Boolean).join(', ');
          setCustomerFormData(prev => ({ ...prev, address: prev.address || endereco }));
        }
        setLoadingCustomerCep(false);
      }).catch(() => setLoadingCustomerCep(false));
    }, 400);
    return () => clearTimeout(t);
  }, [showCustomerModalForm, customerFormCep]);
  ...
  useEffect(() => {
    ...
    // Carregar vendas do storage (remover duplicatas por ID)
    const savedSales = storageManager.get<any[]>('pdv_sales', { defaultValue: [] }) || [];
    const uniqueSales = savedSales.filter((sale, index, self) => 
      index === self.findIndex(s => s.id === sale.id)
    );
    if (uniqueSales.length !== savedSales.length) {
      storageManager.set('pdv_sales', uniqueSales);
    }
  ...
  const handleCreateCustomer = () => {
    ...
    setCustomerFormData({ name: '', email: '', phone: '', cpf: '', address: '', points: 0 });
    setCustomerFormCep('');
  ...
  const handleSaveCustomer = (customerData: Partial<Customer>) => {
    try {
      ...
      if (customerData.cpf) {
        ...
        if (customerData.cpf.length === 14) customerData.cnpj = customerData.cpf;
      }
  ...
  const finalizeSale = async () => {
    ...
    try {
      ...
      // Processar pagamento via gateway (se aplicável) — códigos alinhados ao Movimento de Caixa
      const gatewayRelevant = selectedPaymentMethod && !['dinheiro', 'cash', 'cheque', 'boleto'].includes(selectedPaymentMethod);
      if (gatewayRelevant) {
        try {
          const paymentMethodToGateway: { [key: string]: string } = {
            'cartao': 'credit_card',
            'cartaoLink': 'credit_card',
            'pixMaquininha': 'pix',
            'pixConta': 'pix',
          ...
          const gatewayMethod = paymentMethodToGateway[selectedPaymentMethod] || selectedPaymentMethod;
      ...
      const saleId = `sale_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;
      const newSale = {
        id: saleId,
      ...
      // Atualizar estado e storage de uma vez para evitar duplicação
      setSales(prev => {
        const updated = [newSale, ...prev];
        storageManager.set('pdv_sales', updated);
        return updated;
      });
  ...
  const getPaymentMethodName = (method: string): string => {
    const methods: { [key: string]: string } = {
      ...
      'credit_card': 'Cartão Crédito', 'debit_card': 'Cartão Débito', 'cartao': 'Cartão (crédito/débito)',
      'cartaoLink': 'Cartão Link',
      'pix': 'PIX', 'pixMaquininha': 'PIX Maquininha', 'pixConta': 'PIX Conta',
      'boleto': 'Boleto', 'boletos': 'Boletos',
      'cheque': 'Cheque'
    };
  ...
  const handleImportSales = (importedSales: any[]) => {
    setSales(prev => {
      const updated = [...prev, ...importedSales];
      storageManager.set('pdv_sales', updated);
      return updated;
    });
                ...
                {purchases.length === 0 ? (
                  ...
                  purchases.map((purchase) => (
                    <tr key={purchase.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm">{getPaymentMethodName(purchase.paymentMethod)}</td>
                  ...
                  {expenses.length === 0 ? (
                    ...
                    expenses.map((expense) => (
                      <tr key={expense.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-sm">{getPaymentMethodName(expense.paymentMethod)}</td>
                      </tr>
                    ))
                  )}
                </tbody>
          ...
          <div className="flex space-x-2 overflow-x-auto">
            ...
            <button
              onClick={() => {
                ...
                setEditingCustomer(null);
                setCustomerFormData({ name: '', email: '', phone: '', cpf: '', address: '', points: 0 }); setCustomerFormCep('');
                setShowCustomerModalForm(true);
              }}
              className={`px-4 py-2 rounded-lg transition-colors whitespace-nowrap ${
  ...
  /** Chaves de armazenamento do PDV — zerar todas restaura sistema virgem */
  const PDV_STORAGE_KEYS = [
    'pdv_stores', 'pdv_suppliers', 'pdv_commissions', 'pdv_products', 'pdv_quotations',
    'pdv_purchases', 'pdv_expenses', 'pdv_transfers', 'pdv_users', 'pdv_sellers', 'pdv_babies',
    'pdv_customers', 'pdv_sales', 'pdv_cash_register', 'pdv_cash_closures',
    'pdv_stock_adjustments', 'pdv_stock_counts', 'pdv_sales_cache',
    'pdv_integration_config', 'pdv_nfces', 'pdv_nfes', 'pdv_sat_config',
    'pdv_backup_config', 'pdv_backups',
  ];

  const handleZerarSistemaPDV = () => {
    const msg = 'Tem certeza que deseja ZERAR TODOS os dados do PDV?\n\n' +
      'Serão apagados: produtos, vendas, clientes, caixa, compras, despesas, cotações, ' +
      'transferências, lojas, fornecedores, comissões, usuários, vendedores, NFC-e/NFe e backups.\n\n' +
      'O sistema ficará como novo. Esta ação não pode ser desfeita.';
    if (!window.confirm(msg)) return;
    PDV_STORAGE_KEYS.forEach((key) => {
      storageManager.remove(key);
      try { localStorage.removeItem(key); } catch (_) {}
    });
    pdvIntegrationService.clearSalesCache();
    alert('Todos os dados do PDV foram apagados. A página será recarregada.');
      ...
      <div className="space-y-6">
        <div className="bg-white/95 backdrop-blur-sm rounded-xl shadow-xl border border-gray-200/50 p-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* Zerar sistema - Restaurar para virgem */}
            <div className="bg-gradient-to-br from-white to-red-50 rounded-xl shadow-lg p-6 border border-red-200 lg:col-span-2">
              <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
                <AlertCircle className="w-5 h-5 text-red-600" />
                Restaurar PDV para sistema virgem
              </h3>
              <p className="text-sm text-gray-600 mb-4">
                Remove todos os dados do PDV: produtos, vendas, clientes, caixa, compras, despesas, cotações,
                transferências, lojas, fornecedores, comissões, usuários, vendedores, documentos fiscais e cache de integração.
                O PDV voltará a funcionar como se fosse a primeira utilização. Esta ação não pode ser desfeita.
              </p>
              <button
                type="button"
                onClick={handleZerarSistemaPDV}
                className="flex items-center gap-2 px-4 py-2.5 bg-red-600 text-white rounded-xl hover:bg-red-700 transition-all font-semibold shadow-lg border border-red-700"
              >
  ...
  function StoreForm({ onAdd, onUpdate, editing, onCancelEdit }: { onAdd: (s: Omit<StoreRecord,'id'>) => void; onUpdate: (id: string, s: Omit<StoreRecord,'id'>) => void; editing?: StoreRecord; onCancelEdit: ()=>void; }) {
    const [form, setForm] = useState<Omit<StoreRecord,'id'> & { cep?: string }>({ name:'', address:'', phone:'', email:'', cnpj:'', status:'active', cep:'' });
    const [loadingCnpj, setLoadingCnpj] = useState(false);
    const [loadingCep, setLoadingCep] = useState(false);
    useEffect(()=>{
      if (editing) setForm({ name: editing.name, address: editing.address, phone: editing.phone, email: editing.email, cnpj: editing.cnpj, status: editing.status, cep: '' });
    }, [editing]);
    useEffect(() => {
        ...
        cnpjService.consultarCNPJ(clean).then((data) => {
          if (data) {
            setForm(prev => ({
              ...prev,
              name: prev.name || data.nome_fantasia || data.razao_social || '',
              address: prev.address || [data.logradouro, data.numero, data.bairro, data.municipio, data.uf].filter(Boolean).join(', '),
              phone: prev.phone || data.telefone || '',
              email: prev.email || data.email || '',
            }));
          }
          setLoadingCnpj(false);
        }).catch(() => setLoadingCnpj(false));
      }, 500);
      return () => clearTimeout(t);
    }, [form.cnpj]);
    useEffect(() => {
      ...
      const t = setTimeout(() => {
        ...
        cepService.buscarCEP(clean).then((data) => {
          if (data) {
            const endereco = [data.logradouro, data.bairro, data.localidade, data.uf].filter(Boolean).join(', ');
            setForm(prev => ({ ...prev, address: prev.address || endereco }));
          }
          setLoadingCep(false);
    ...
    const maskCEP = (v: string) => v.replace(/\D/g,'').replace(/(\d{5})(\d{1,3})/, '$1-$2').slice(0, 9);
    return (
      <div className="bg-gray-50 rounded-xl p-4">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          ...
          <div className="relative">
            <input className="w-full px-3 py-2 border rounded pr-8" placeholder="CNPJ" value={form.cnpj||''} onChange={e=>setForm({...form,cnpj:maskCNPJ(e.target.value)})}/>
            {loadingCnpj && <div className="absolute right-2 top-1/2 -translate-y-1/2"><Loader2 className="w-4 h-4 text-blue-500 animate-spin" /></div>}
          </div>
        ...
        <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="relative">
            <input className="w-full px-3 py-2 border rounded pr-8" placeholder="CEP (busca automática)" value={form.cep||''} onChange={e=>setForm({...form,cep:maskCEP(e.target.value)})} maxLength={9}/>
            {loadingCep && <div className="absolute right-2 top-1/2 -translate-y-1/2"><Loader2 className="w-4 h-4 text-blue-500 animate-spin" /></div>}
          </div>
        ...
        <div className="mt-3 flex justify-end">
          {editing ? (
            <div className="flex gap-2">
              <button onClick={()=>{ if(!form.name.trim()||!editing) return; onUpdate(editing.id, { name: form.name, address: form.address, phone: form.phone, email: form.email, cnpj: form.cnpj, status: form.status }); onCancelEdit(); setForm({ name:'', address:'', phone:'', email:'', cnpj:'', status:'active', cep:'' }); }} className="px-4 py-2 bg-blue-600 text-white rounded">Salvar</button>
              <button onClick={()=>{ onCancelEdit(); setForm({ name:'', address:'', phone:'', email:'', cnpj:'', status:'active', cep:'' }); }} className="px-4 py-2 bg-gray-200 text-gray-700 rounded">Cancelar</button>
            </div>
          ) : (
            <button onClick={()=>{ if(!form.name.trim()) return; onAdd({ name: form.name, address: form.address, phone: form.phone, email: form.email, cnpj: form.cnpj, status: form.status }); setForm({ name:'', address:'', phone:'', email:'', cnpj:'', status:'active', cep:'' }); }} className="px-4 py-2 bg-green-600 text-white rounded">Adicionar Loja</button>
          )}
        </div>
      </div>
    );
        ...
        cnpjService.consultarCNPJ(clean).then((data) => {
          if (data) {
            setForm(prev => ({
              ...prev,
              name: prev.name || data.nome_fantasia || data.razao_social || '',
              contact: prev.contact || data.razao_social || '',
              phone: prev.phone || data.telefone || '',
              email: prev.email || data.email || '',
            }));
          }
              ...
              <div className="p-6">
                <div className="space-y-4">
                  <div>
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1">
                            ...
                            {empresaData.razao_social && empresaData.razao_social !== empresaData.nome_fantasia && (
                              <p className="text-xs text-green-700 mb-1">Razão Social: {empresaData.razao_social}</p>
                            )}
                    ...
                    {(() => {
                      const cleanCnpj = customerCpfCnpj.replace(/\D/g, '');
                      const isCNPJ = cleanCnpj.length === 14;
                      if (!empresaData || !isCNPJ) return null;
                      
                      // Verificar se já está cadastrado usando o estado atual
                      const jaCadastrado = customers.some(c => {
                        const cDoc = String(c.cpf || c.cnpj || '').replace(/\D/g, '');
                        return cDoc === cleanCnpj;
                      });
                      
                      if (jaCadastrado) {
                        return (
                          <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                            <p className="text-sm text-blue-800">✓ Este CNPJ já está cadastrado como cliente.</p>
                          </div>
                        );
                      }
                      
                      const endereco = [empresaData.logradouro, empresaData.numero, empresaData.bairro, empresaData.municipio, empresaData.uf].filter(Boolean).join(', ');
                      return (
                        <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                          <p className="text-sm text-amber-800 mb-2">Este CNPJ ainda não está cadastrado como cliente.</p>
                          <button
                            type="button"
                            onClick={() => {
                              try {
                                const newCustomer: Customer = {
                                  id: `cnpj_${cleanCnpj}_${Date.now()}`,
                                  name: empresaData.nome_fantasia || empresaData.razao_social || '',
                                ...
                                setCustomers(prev => {
                                  ...
                                  if (exists) {
                                    alert('Este CNPJ já está cadastrado como cliente.');
                                    return prev;
                                  }
                                  const updated = [...prev, newCustomer];
                                  storageManager.set('pdv_customers', updated);
                                  return updated;
                                });
                            ...
                            className="w-full px-3 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 text-sm font-medium transition-colors"
                          >
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    >
                      ...
                      <option value="cartaoLink">Cartão Link</option>
                      <option value="pixMaquininha">PIX Maquininha</option>
                      <option value="pixConta">PIX Conta</option>
                      <option value="boleto">Boleto</option>
                      <option value="cheque">Cheque</option>
                    </select>
                  </div>
                  
                  {(selectedPaymentMethod === 'dinheiro' || selectedPaymentMethod === 'cash') && (
                    <div>
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-6">
                <form onSubmit={(e) => {
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">CPF/CNPJ (busca automática)</label>
                    <div className="relative">
                      <input
                        type="text"
                        name="cpf"
                        ...
                        maxLength={18}
                      />
                      {loadingCustomerCnpj && <div className="absolute right-2 top-1/2 -translate-y-1/2"><Loader2 className="w-5 h-5 text-blue-500 animate-spin" /></div>}
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">CEP (busca automática)</label>
                    <div className="relative">
                      <input
                        type="text"
                        name="cep"
                        value={customerFormCep.replace(/\D/g,'').replace(/(\d{5})(\d{1,3})/, '$1-$2').slice(0, 9)}
                        onChange={(e) => setCustomerFormCep(e.target.value.replace(/\D/g, '').slice(0, 8))}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent pr-10"
                        placeholder="00000-000"
                        maxLength={9}
                      />
                      {loadingCustomerCep && <div className="absolute right-2 top-1/2 -translate-y-1/2"><Loader2 className="w-5 h-5 text-blue-500 animate-spin" /></div>}
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Endereço</label>
                          ...
                          const importedSales = lines.slice(1)
                            ...
                            .map(line => {
                              ...
                              return {
                                ...
                                paymentMethod: values[headers.indexOf('paymentMethod')] || values[5] || 'dinheiro'
                              };
                            });
