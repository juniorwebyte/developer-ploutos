...
// CNPJ principal do sistema - apenas SuperAdmin pode alterar
const CNPJ_PRINCIPAL = '29793949000178';
...
export default function OwnerPanel({ isOpen, onClose, onConfigUpdate }: OwnerPanelProps) {
  ...
  const { user, role, setLicense: setAuthLicense } = useAuth();
  ...
  const [cepEncontrado, setCepEncontrado] = useState<boolean | null>(null);
  const [erroCEP, setErroCEP] = useState<string>('');
  
  // Estados para busca de CNPJ
  const [buscandoCNPJ, setBuscandoCNPJ] = useState(false);
  const [cnpjEncontrado, setCnpjEncontrado] = useState<boolean | null>(null);
  const [erroCNPJ, setErroCNPJ] = useState<string>('');
  
  // Verificar se é Super Admin
  const isSuperAdmin = role === 'superadmin' || user === 'superadmin';
  
  // Verificar se o CNPJ é o principal (bloqueado para edição)
  const isCNPJPrincipal = config.cnpj.replace(/\D/g, '') === CNPJ_PRINCIPAL;
    ...
    try {
      ...
      if (resultado) {
        ...
        setConfig(prev => ({
          ...
          endereco: {
            ...prev.endereco,
            logradouro: resultado.logradouro || prev.endereco.logradouro,
            bairro: resultado.bairro || prev.endereco.bairro,
            cidade: resultado.localidade || prev.endereco.cidade,
            estado: resultado.uf || prev.endereco.estado,
            complemento: resultado.complemento || prev.endereco.complemento,
            cep: cepLimpo
          }
        }));
  ...
  const handleBuscarCNPJ = useCallback(async (cnpj: string) => {
    ...
    // Não buscar se for o CNPJ principal e não for SuperAdmin
    if (cnpjLimpo === CNPJ_PRINCIPAL && !isSuperAdmin) {
      setErroCNPJ('CNPJ principal. Apenas o SuperAdmin pode alterar.');
      return;
    }
    ...
    try {
      const resultado = await buscarCNPJ(cnpjLimpo);
      
      if (resultado) {
        // Atualizar campos da empresa
        setConfig(prev => ({
          ...prev,
          nomeFantasia: resultado.fantasia || resultado.nome || prev.nomeFantasia,
          razaoSocial: resultado.nome || prev.razaoSocial,
          cnpj: cnpjLimpo,
          endereco: {
            logradouro: resultado.logradouro || prev.endereco.logradouro,
            numero: resultado.numero || prev.endereco.numero,
            complemento: resultado.complemento || prev.endereco.complemento,
            bairro: resultado.bairro || prev.endereco.bairro,
            cidade: resultado.municipio || prev.endereco.cidade,
            estado: resultado.uf || prev.endereco.estado,
            cep: resultado.cep?.replace(/\D/g, '') || prev.endereco.cep
          },
          contato: {
            ...prev.contato,
            telefone: resultado.telefone || prev.contato.telefone,
            email: resultado.email || prev.contato.email
          }
        }));
        ...
        setCnpjEncontrado(false);
        setErroCNPJ('CNPJ não encontrado ou inválido');
      }
    } catch (error) {
      console.error('Erro ao buscar CNPJ:', error);
      setCnpjEncontrado(false);
      setErroCNPJ('Erro ao buscar CNPJ. Tente novamente.');
    } finally {
  ...
  useEffect(() => {
    const cepLimpo = config.endereco.cep.replace(/\D/g, '');
    if (cepLimpo.length === 8 && !buscandoCEP) {
      // Delay para não fazer muitas requisições enquanto digita
      const timer = setTimeout(() => {
        handleBuscarCEP(cepLimpo);
      }, 500);
      return () => clearTimeout(timer);
    }
  }, [config.endereco.cep, handleBuscarCEP, buscandoCEP]);

  // Efeito para buscar CNPJ automaticamente quando tiver 14 dígitos
  useEffect(() => {
    const cnpjLimpo = config.cnpj.replace(/\D/g, '');
    if (cnpjLimpo.length === 14 && !buscandoCNPJ && cnpjLimpo !== CNPJ_PRINCIPAL) {
      // Delay para não fazer muitas requisições enquanto digita
      const timer = setTimeout(() => {
        handleBuscarCNPJ(cnpjLimpo);
      }, 500);
      return () => clearTimeout(timer);
    }
  }, [config.cnpj, handleBuscarCNPJ, buscandoCNPJ]);
        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 min-h-0" style={{ maxHeight: 'calc(95vh - 280px)' }}>
          {activeTab === 'empresa' && (
            <div className="space-y-6">
              {/* Dica sobre busca automática */}
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  <Search className="w-5 h-5 text-blue-600 mt-0.5" />
                  <div>
                    <p className="text-sm font-medium text-blue-900">Preenchimento Automático</p>
                    <p className="text-xs text-blue-700 mt-1">
                      Digite o CNPJ completo para buscar automaticamente os dados da empresa na Receita Federal.
                      O CEP também preenche automaticamente os campos de endereço.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Nome Fantasia *
                    {cnpjEncontrado && config.nomeFantasia && (
                  ...
                  <input
                    ...
                    className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-300 ${
                      cnpjEncontrado && config.nomeFantasia ? 'bg-green-50 border-green-200' : 'border-gray-300'
                    }`}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Razão Social *
                    {cnpjEncontrado && config.razaoSocial && (
                      <span className="text-xs bg-green-100 text-green-800 px-2 py-0.5 rounded-full ml-2">
                        Via CNPJ
                      </span>
                    )}
                  ...
                  <input
                    ...
                    className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-300 ${
                      cnpjEncontrado && config.razaoSocial ? 'bg-green-50 border-green-200' : 'border-gray-300'
                    }`}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center gap-2">
                    CNPJ *
                    {isCNPJPrincipal && !isSuperAdmin && (
                      <span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded-full">
                        CNPJ Principal (bloqueado)
                      </span>
                    )}
                    {isSuperAdmin && (
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => {
                        if (isCNPJPrincipal && !isSuperAdmin) return;
                        const unformatted = unformatCNPJ(e.target.value);
                        handleInputChange('cnpj', unformatted);
                        setCnpjEncontrado(null);
                        setErroCNPJ('');
                      }}
                      disabled={isCNPJPrincipal && !isSuperAdmin}
                      className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:border-transparent transition-all duration-300 pr-10 ${
                        isCNPJPrincipal && !isSuperAdmin 
                    ...
                    <div className="absolute right-3 top-1/2 -translate-y-1/2">
                      {buscandoCNPJ && <Loader2 className="w-5 h-5 text-blue-500 animate-spin" />}
                      {!buscandoCNPJ && cnpjEncontrado === true && <CheckCircle2 className="w-5 h-5 text-green-500" />}
                      {!buscandoCNPJ && cnpjEncontrado === false && <AlertCircle className="w-5 h-5 text-red-500" />}
                      {!buscandoCNPJ && cnpjEncontrado === null && config.cnpj.replace(/\D/g, '').length === 14 && !(isCNPJPrincipal && !isSuperAdmin) && (
                        <button
                          type="button"
                          onClick={() => handleBuscarCNPJ(config.cnpj)}
                  ...
                  {erroCNPJ && <p className="text-red-600 text-xs mt-1">{erroCNPJ}</p>}
                  {cnpjEncontrado && <p className="text-green-600 text-xs mt-1">Dados da empresa preenchidos automaticamente!</p>}
                  {isCNPJPrincipal && !isSuperAdmin && (
                    <p className="text-yellow-700 text-xs mt-1">
                      Este é o CNPJ principal do sistema. Apenas o SuperAdmin pode alterá-lo.
                    </p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent ${
                        cepEncontrado && config.endereco.logradouro ? 'bg-green-50 border-green-200' : 'border-gray-300'
                      }`}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent ${
                        cepEncontrado && config.endereco.bairro ? 'bg-green-50 border-green-200' : 'border-gray-300'
                      }`}
                      placeholder="Centro"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent ${
                        cepEncontrado && config.endereco.cidade ? 'bg-green-50 border-green-200' : 'border-gray-300'
                      }`}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent ${
                        cepEncontrado && config.endereco.estado ? 'bg-green-50 border-green-200' : 'border-gray-300'
                      }`}
                    >
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      />
                      <div className="absolute right-3 top-1/2 -translate-y-1/2">
                        {buscandoCEP && <Loader2 className="w-5 h-5 text-blue-500 animate-spin" />}
                        {!buscandoCEP && cepEncontrado === true && <CheckCircle2 className="w-5 h-5 text-green-500" />}
                        {!buscandoCEP && cepEncontrado === false && <AlertCircle className="w-5 h-5 text-red-500" />}
                        {!buscandoCEP && cepEncontrado === null && config.endereco.cep.replace(/\D/g, '').length === 8 && (
                          <button
                            type="button"
                            onClick={() => handleBuscarCEP(config.endereco.cep)}
                            className="p-1 hover:bg-gray-100 rounded"
                    ...
                    {erroCEP && <p className="text-red-600 text-xs mt-1">{erroCEP}</p>}
                    {cepEncontrado && <p className="text-green-600 text-xs mt-1">Endereço preenchido automaticamente!</p>}
                  </div>
                </div>
              </div>
