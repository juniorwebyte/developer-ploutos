...
export const useCashFlow = () => {
  ...
  const totalSangrias = useMemo(() => {
    if (!Array.isArray(exits.sangrias)) return 0;
    return exits.sangrias.reduce((sum, s) => preciseCurrency.add(sum, Number(s.valor) || 0), 0);
  }, [exits.sangrias]);
  ...
  const totalFinal = useMemo(() => {
    ...
    return preciseCurrency.subtract(
      preciseCurrency.subtract(entradasComAdicionais, totalSaidasRetiradas),
      totalSangrias
    );
  }, [totalEntradas, totalCheques, totalDevolucoes, totalEnviosCorreios, valesImpactoEntrada, totalSaidasRetiradas, totalSangrias]);
  ...
  const adicionarSangria = useCallback((sangria: Omit<Sangria, 'id'>) => {
    const nova: Sangria = {
      ...sangria,
      id: `sangria-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`
    };
    setExits(prev => ({
      ...prev,
      sangrias: [...(prev.sangrias || []), nova]
    }));
    setHasChanges(true);
    return nova;
  }, []);
      ...
      try {
        ...
        if (parsed.entries && parsed.exits) {
          ...
          const exitsWithDefault = {
            ...
            // Sangria de Caixa
            sangrias: Array.isArray(parsed.exits.sangrias) ? parsed.exits.sangrias : [],
